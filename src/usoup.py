...
def get_week_schedule(file_path):
    ...
    info_tag = soup.find(id='lblRoomDetails')
    days['info'] = ""
    if (len(info_tag.contents)>2):
        days['info'] = info_tag.contents[2]
    return days

'''days = {
    room_name: 'BUILDING_####asdf'
    m: {
...
week_calendar = get_week_schedule('../rooms/html_data.txt')

print(week_calendar['info'])
